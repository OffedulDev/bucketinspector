...
func _use_cup() -> void:
	if uses == MAX_USES: 
		inspector.prompt_message("""[font_size=20]The cup has NOT spoken![/font_size]
...
is bringing [b][color=pink]IS """ + (inspector.current_npc_information["real_content"] if inspector.current_npc_information.get("real_content") else inspector.current_npc_information["content"]) + 
"[/color][/b]")
